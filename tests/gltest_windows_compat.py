...
def install_windows_direct_compatibility() -> None:
    ...
    if getattr(loader, "_semantic_mutex_windows_compat", False):
        return
    ...
    def inject_message_to_fd0(vm):
        ...
        try:
            ...
            os.dup2(fd, 0)
            vm._semantic_mutex_stdin_path = path
        finally:
            os.close(fd)
    ...
    def cleanup_after_deactivate(vm):
        path = getattr(vm, "_semantic_mutex_stdin_path", None)
        try:
            original_cleanup(vm)
        finally:
            if path:
                try:
                    ...
                    pass
                vm._semantic_mutex_stdin_path = None
...
def install_glsim_direct_compatibility() -> None:
    ...
    engine_module._semantic_mutex_direct_compat = True
